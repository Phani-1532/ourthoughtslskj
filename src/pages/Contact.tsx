      ...
      <div className="pt-24 bg-background">
        <SectionWrapper>
          ...
          <div className="grid lg:grid-cols-5 gap-6 lg:gap-10">
            {/* Left column: Contact Info + AI Assistant */}
                  ...
                  <Card key={c.label} className="bg-card/80 border-border/30 shadow-card hover:border-primary/50 transition-colors">
                    <CardContent className="p-4 flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {c.href ? (
                          <a href={c.href} target="_blank" rel="noopener noreferrer" className="text-sm text-primary font-body font-medium truncate block">{c.value}</a>
                        ) : (
                          <p className="text-sm text-foreground font-body font-medium truncate">{c.value}</p>
                        )}
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <div className="flex items-center justify-between mb-6 md:mb-8">
                    {steps.map((s, i) => (
                      <div key={s} className="flex items-center">
                        <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center text-xs md:text-sm font-body font-medium transition-colors ${
                          i <= step ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground"
                        }`}>{i + 1}</div>
                        <span className="hidden md:block text-xs font-body text-muted-foreground ml-2">{s}</span>
                        {i < steps.length - 1 && <div className={`w-4 md:w-12 lg:w-16 h-0.5 mx-1 md:mx-2 ${i < step ? "bg-primary" : "bg-secondary"}`} />}
                      </div>
                    ))}
                  </div>
                  ...
                  <AnimatePresence mode="wait">
                    <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
                      ...
                      {step === 2 && (
                        <div className="space-y-4">
                          <h3 className="text-lg md:text-xl font-display font-semibold text-foreground mb-4 flex items-center gap-2">
                            <Briefcase className="w-5 h-5 text-primary" />Project Requirements
                          </h3>
                      ...
                      {step === 3 && (
                        <div className="space-y-4">
                          <h3 className="text-lg md:text-xl font-display font-semibold text-foreground mb-4 flex items-center gap-2">
                            <Send className="w-5 h-5 text-primary" />Review & Submit
                          </h3>
                          <div className="bg-secondary/30 rounded-xl p-4 md:p-6 space-y-3">
                            {[
                              ...
                              <div key={x.l} className="flex justify-between text-sm font-body gap-4">
                                <span className="text-muted-foreground shrink-0">{x.l}</span>
                                <span className="text-foreground text-right">{x.v}</span>
                              </div>
                            ))}
