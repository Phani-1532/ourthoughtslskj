      ...
      {services.map((service, i) => (
        ...
        >
          <Link to={service.path} className="block group h-full">
            <div className="gradient-border relative p-6 rounded-2xl border border-border bg-card h-full overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-elevated">
              {/* Glow accent on hover */}
              <div className="absolute -top-20 -right-20 w-40 h-40 rounded-full bg-primary/0 group-hover:bg-primary/8 transition-colors duration-500 blur-2xl" />
              {/* Number watermark */}
              <span className="absolute top-3 right-4 text-5xl font-extrabold text-foreground/[0.03] group-hover:text-primary/10 transition-colors duration-500 select-none">
                {String(i + 1).padStart(2, "0")}
              </span>
              <div className="relative">
                <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center mb-5 group-hover:bg-primary group-hover:shadow-glow transition-all duration-300 group-hover:scale-110">
                  <service.icon className="w-5 h-5 text-primary group-hover:text-primary-foreground transition-colors" />
                </div>
                <h3 className="text-sm font-bold text-foreground mb-2 group-hover:text-primary transition-colors">{service.title}</h3>
